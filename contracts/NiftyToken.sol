...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    // Array of addresses of allowed markets
    address[] private _marketAddresses;

    // Mapping of addresses of allowed markets
    ...
    function approve(address to, uint256 tokenId) public virtual override {
        ...
        if (_marketAddresses.length > 0) {
            require(
                marketAllowList[to] == true,
                "ERC721: approval of address not allowed"
            );
        }
        ...
        override
    {
        ...
        if (approved == true && _marketAddresses.length > 0) {
            require(
                marketAllowList[operator] == true,
                "ERC721: approval of address not allowed"
            );
        }
    ...
    function _seedMarketAllowList(address[] memory addresses) internal virtual {
        for (uint256 i = 0; i < _marketAddresses.length; i++) {
            delete marketAllowList[_marketAddresses[i]];
        }

        delete _marketAddresses;

        for (uint256 i = 0; i < addresses.length; i++) {
            _marketAddresses.push(addresses[i]);
            marketAllowList[addresses[i]] = true;
        }
    }
...
abstract contract ERC2981PerTokenRoyalties is ERC2981 {
    ...
    function setTokenRoyaltyRecipient(uint256 _tokenId, address _recipient)
        external
    {
        // the royalty must be set and the caller must be the current recipient of the royalty
        RoyaltyInfo memory royalty = _royalties[_tokenId];
        require(
            royalty.recipient != address(0) && royalty.recipient == msg.sender,
            "ERC2981PerTokenRoyalties: Current recipient only"
        );
    ...
    /// @param _recipient recipient of the royalties
    /// @param _value percentage (using 2 decimals - 10000 = 100, 0 = 0)
    function _setRoyalty(address _recipient, uint256 _value) internal {
        require(_value <= 10000, "ERC2981PerTokenRoyalties: Royalty too high");
...
contract NiftyToken is ERC721URIStorage, ERC2981PerTokenRoyalties, Ownable {
    ...
    // Set collection-wide default royalty.
    function setRoyalty(address _receiver, uint16 _royaltyPercent)
        external
        override
        onlyOwner
    {
    ...
    function setTokenRoyalty(
        ...
        address operator = _msgSender();
        require(
            creatorOf(_tokenId) == operator || isApproved(_tokenId, operator),
            "Only owner or approved"
        );
    ...
    /**
     * @dev returns the creator of a tokenId
     */
    ...
    /**
     @notice Method for updating the market list to blcok certain addresses
     @dev Only admin
     @param addresses is the array of addresses to allow. Leave empty to clear the list
     */
    function seedMarketAllowList(address[] memory addresses)
        external
        onlyOwner
    {
        _seedMarketAllowList(addresses);
    }
