// test/NiftyFairAuctionV2.test.js
// This test is designed to run through a full scenario of the `NiftyAuction` contract
...
contract('NiftyAuction', async function () {
  before(async function () {
    ...
    NiftyAuction = await ethers.getContractFactory('MockNiftyFairAuction');
    NiftyToken = await ethers.getContractFactory('NiftyToken');
    NiftyMarketplace = await ethers.getContractFactory('NiftyMarketplace');
    NiftyBundleMarketplace = await ethers.getContractFactory(
      'NiftyBundleMarketplace'
    );
    NiftyAddressRegistry = await ethers.getContractFactory(
      'NiftyAddressRegistry'
    );
    NiftyTokenRegistry = await ethers.getContractFactory('NiftyTokenRegistry');
    NiftyRoyaltyRegistry = await ethers.getContractFactory(
      'NiftyRoyaltyRegistry'
    );
    NiftyPriceFeed = await ethers.getContractFactory('NiftyPriceFeed');
    ...
    niftyfairauction = await NiftyAuction.deploy();
    await niftyfairauction.deployed();
    niftyfairartion = await NiftyToken.deploy(owner.address, ONE);
    await niftyfairartion.deployed();
    niftyfairmarketplace = await NiftyMarketplace.deploy();
    await niftyfairmarketplace.deployed();
    niftyfairbundlemarketplace = await NiftyBundleMarketplace.deploy();
    await niftyfairbundlemarketplace.deployed();
    niftyfairaddressregistry = await NiftyAddressRegistry.deploy();
    await niftyfairaddressregistry.deployed();
    niftyfairtokenregistry = await NiftyTokenRegistry.deploy();
    await niftyfairtokenregistry.deployed();
    niftyfairRoyaltyRegistry = await NiftyRoyaltyRegistry.deploy();
    await niftyfairtokenregistry.deployed();
    niftyfairPriceFeed = await NiftyPriceFeed.deploy(
      niftyfairaddressregistry.address,
      mockerc20.address
    );
    await niftyfairPriceFeed.deployed();
  ...
  afterEach(async function () {
    ...
    niftyfairauction.connect(owner).hardhatTimestamp(timeNow);
  });

  describe('NiftyAuction extensive unit test', function () {
    it('005) cannot pause auction contract if not `owner`', async function () {
      await expectRevert(
        niftyfairauction.connect(other).toggleIsPaused(),
    ...
    it('007) test auction created successfully for user `seller`', async function () {
      await expect(
        ...
        .to.emit(niftyfairauction, 'AuctionCreated')
    ...
    it('008) NFT successfully in escrow with auction contract', async function () {
      ...
      expect(result.toString()).to.equal(niftyfairauction.address);
    });
    ...
    it('009) created auction `seller` is `_owner`', async function () {
      ...
      const result = await niftyfairauction
        .connect(seller)
    ...
    // Test case **ID: A13**:: Attempt to list an auction with an `_endTime` of `0` rework
    ...
    it('020) cannot cancel an auction not owned by the auction contract', async function () {
      await expect(
        niftyfairauction.connect(seller).cancelAuction(mockerc721.address, 1)
    ...
    it('021) cannot cancel an auction you do not own', async function () {
      await expect(
        niftyfairauction.connect(other).cancelAuction(mockerc721.address, 4)
      ).to.be.revertedWith('sender must be owner');
    });
    ...
    it('022) test auction cancelled successfully by user `seller`', async function () {
      await expect(
        niftyfairauction.connect(seller).cancelAuction(mockerc721.address, 4)
      )
        .to.emit(niftyfairauction, 'AuctionCancelled')
    ...
    it('blockchain time increased 100 seconds', async function () {
      ...
      time.advanceBlock();
      expect(
        (await niftyfairauction.connect(owner).owner()).toString()
    ...
    it('028) cannot place a bid of zero', async function () {
      await expect(
        niftyfairauction
          .connect(bidder)
          .placeBid(mockerc721.address, FOUR, ZERO)
    ...
    it('030) bid successfully placed at `minBidIncrement`', async function () {
      ...
      await expect(
        ...
        .to.emit(niftyfairauction, 'BidPlaced')
    ...
    it('032) auction contract has the correct amount of `MockERC20` tokens', async function () {
      ...
      // Test if the ERC20 pay tokens were refunded properly
      expect(
        await mockerc20.connect(owner).balanceOf(niftyfairauction.address)
      ).to.be.bignumber.equal(amount);
    });
    ...
    it('035) auction contract cannot cancel an auction they dont own', async function () {
      ...
      await expect(
        niftyfairauction.connect(owner).cancelAuction(mockerc721.address, 4)
      ).to.be.revertedWith('sender must be owner');
    });
    ...
    it('048) cannot result an auction that hasnt ended as `seller`', async function () {
      await expect(
        niftyfairauction.connect(seller).resultAuction(mockerc721.address, 4)
    ...
    it('049) cannot result an auction that hasnt ended as `other`', async function () {
      await expect(
        niftyfairauction.connect(other).resultAuction(mockerc721.address, 4)
    ...
    it('050) cannot result a finished auction that ended with bids below the reserve price', async function () {
      await expect(
        niftyfairauction.connect(bidder).resultAuction(mockerc721.address, 4)
    ...
    it('051) cannot result a finished auction that ended with bids below the reserve price as non-owner', async function () {
      await expect(
        niftyfairauction.connect(hacker).resultAuction(mockerc721.address, 4)
      ).to.be.revertedWith('_msgSender() must be auction winner or seller');
    });
    ...
    it('052) cannot cancel an auction that has ended with bids below reserve price as `bidder` or `winner` (must be arranged via resultFailedAuction())', async function () {
      await expect(
        niftyfairauction.connect(bidder).cancelAuction(mockerc721.address, 4)
      ).to.be.revertedWith('sender must be owner');
    });
    ...
    it('064) cannot cancel an auction that has ended with bids >= reserve price as `winner`', async function () {
      await expect(
        niftyfairauction.connect(winner).cancelAuction(mockerc721.address, 4)
      ).to.be.revertedWith('sender must be owner');
    });
    ...
    it('065) cannot resultFailedAuction() an auction that has met reserve price as `seller`', async function () {
      await expect(
        niftyfairauction
          .connect(seller)
          .resultFailedAuction(mockerc721.address, 4)
    ...
    it('070) cannot result a successful auction as `other`', async function () {
      ...
      time.advanceBlock();
      await expect(
        niftyfairauction.connect(other).resultAuction(mockerc721.address, FOUR)
      ).to.be.revertedWith('_msgSender() must be auction winner or seller');
    });
    ...
    it('071) test auction `_tokenId(4)` successfully resulted by `seller`', async function () {
      ...
      time.advanceBlock();
      await expect(
        niftyfairauction.connect(seller).resultAuction(mockerc721.address, FOUR)
      )
        .to.emit(niftyfairauction, 'AuctionResulted')
        .withArgs(
          seller.address,
          mockerc721.address,
          FOUR,
    ...
    it('074) cannot result an auction that has already been resulted as `winner`', async function () {
      await expect(
        niftyfairauction.connect(winner).resultAuction(mockerc721.address, FOUR)
      ).to.be.revertedWith('_msgSender() must be auction winner or seller');
    });
    ...
    it('077) cannot cancel an auction that has ended successfully and has been resulted as `seller`', async function () {
      await expect(
        niftyfairauction.connect(seller).cancelAuction(mockerc721.address, FOUR)
      ).to.be.revertedWith('sender must be owner');
    });
    ...
    it('078) cannot cancel an auction that has ended successfully and has been resulted as `other`', async function () {
      await expect(
        niftyfairauction.connect(other).cancelAuction(mockerc721.address, FOUR)
      ).to.be.revertedWith('sender must be owner');
    });
    ...
    it('085) test auction `_tokenId(3)` successfully resulted by `winner`', async function () {
      await expect(
        niftyfairauction.connect(seller).resultAuction(mockerc721.address, FIVE)
      )
        .to.emit(niftyfairauction, 'AuctionResulted')
        .withArgs(
          seller.address,
    ...
    it('091) cannot withdraw a bid if youre not the current highest bidder', async function () {
      await expect(
        niftyfairauction.connect(hacker).withdrawBid(mockerc721.address, TWO)
    ...
    it('092) `bidder` cannot withdraw a bid before end of the auction even when bid is below reserve price', async function () {
      await expect(
        niftyfairauction.connect(bidder).withdrawBid(mockerc721.address, TWO)
      ).to.be.revertedWith(
    ...
    it('094) `seller` cannot raise the auction reserve price', async function () {
      const greaterReservePrice = new BigNumber.from('101000000000000000000'); // 101 WXDAI
      await expect(
        niftyfairauction
          .connect(seller)
          .updateAuctionReservePrice(
    ...
    it('095) `seller` successfully lowered reserve price to `sellerNewReservePrice`', async function () {
      await expect(
        ...
        .to.emit(niftyfairauction, 'UpdateAuctionReservePrice')
        .withArgs(
          mockerc721.address,
          TWO,
          mockerc20.address,
    ...
    it('099 `bidder` successfully withdrew bid once grace window started', async function () {
      await expect(
        ...
        .to.emit(niftyfairauction, 'BidWithdrawn')
        .withArgs(
          mockerc721.address,
          TWO,
          bidder.address,
    ...
    it('100) cannot result an auction that ended and had the highest bidder withdraw', async function () {
      await expect(
        niftyfairauction.connect(seller).resultAuction(mockerc721.address, TWO)
    ...
    it('101) successfully cancelled auction that ended successfully but had the bidder withdraw', async function () {
      await expect(
        niftyfairauction.connect(seller).cancelAuction(mockerc721.address, TWO)
      )
        .to.emit(niftyfairauction, 'AuctionCancelled')
